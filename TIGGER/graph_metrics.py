...
def get_edges_from_adj_graph(graph):
    ...
    for start, adj_list in graph.items():
        for end, value in adj_list.items():
            if value > 0:
                start_v, end_v = min(start,end), max(start,end)
                s.add("_".join([str(int(start_v)), str(int(end_v))]))
    return s
...
graph_label_to_id = pickle.load(open(config_path + "/graph_label_to_id.pkl", "rb"))
...
for label in sampled_graphs.keys():
    for snapshot_idx in sampled_graphs[label].keys():
        ...
        if original_matrix.shape[0] > 2 and sampled_matrix.shape[0] > 2:
            ...
            logging.info("skipping graph with less than 10 nodes, orig: %d, sampled: %d" % (original_matrix.shape[0], sampled_matrix.shape[0]))
