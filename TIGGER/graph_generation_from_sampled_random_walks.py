...
pickle.dump(
    original_graphs,
    open(config_dir + f"/results_{model_name}/original_graphs.pkl", "wb"),
)
pickle.dump(
    time_labels, open(config_dir + f"/results_{model_name}/time_labels.pkl", "wb")
)
pickle.dump(max_day, open(config_dir + f"/results_{model_name}/max_days.pkl", "wb"))
...
for i in range(0, num_of_sampled_graphs):
    ...
    for lb_idx, label in enumerate(sorted(generated_events.keys())):
        ...
        fp = open(config_dir + f"/results_{model_name}/sampled_graph_{i}.pkl", "wb")
