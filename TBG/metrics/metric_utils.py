...
def get_adj_origina_graph_from_original_temporal_graph(temporal_original_graph,start_time,end_time):
    ...
    for time in range(start_time,end_time+1):
        #print(time)
        if time not in temporal_original_graph:
            continue
...
def get_total_nodes_and_edges_from_temporal_adj_list_in_time_range(temporal_adj_matrix,start_time,end_time):
    ...
    unique_nodes = set()
    for time in range(start_time,end_time+1,1):
        if time not in temporal_adj_matrix:
            continue
        for start_node, adj_list in temporal_adj_matrix[time].items():
...
def sample_adj_graph_topk(graph,start,end,edge_count_needed,node_count_needed,req_deg_seq=None,undirected=True):
    """
    Samples a subgraph from the given temporal graph based on the top-k edges and node degree sequence requirements.
    Parameters:
    graph (dict): The input temporal graph where keys are timestamps and values are adjacency lists.
    start (int): The start time for sampling.
    end (int): The end time for sampling.
    edge_count_needed (int): The required number of edges in the sampled subgraph.
    node_count_needed (int): The required number of nodes in the sampled subgraph.
    req_deg_seq (list): The required degree sequence for the nodes in the sampled subgraph.
    undirected (bool, optional): If True, treats the graph as undirected. Defaults to True.
    Returns:
    dict: The sampled subgraph represented as a dictionary of dictionaries.
    """
    req_edge_ct = edge_count_needed
    req_node_ct = node_count_needed
    #print("Required edges count and node count , " ,req_edge_ct,req_node_ct)
    if req_deg_seq is not None: 
        req_deg_seq.sort(reverse=True)
    #print(req_deg_seq)
    tedges = {}
    for time in range(start,end+1):
    ...
    for edge,ct in tedges.items():
        edge = edge.split("#")
        start_node,end_node = edge[0],edge[1]
        uv_str = "#".join([start_node,end_node])
        node_interaction_ct[start_node][end_node] += ct
        node_interaction_ct[end_node][start_node] += ct
        if uv_str not in edges:
            edges[uv_str] = ct
        else:
            edges[uv_str] += ct
...
def sample_adj_graph_topk_continuous(graph,start,end,
        edge_count_needed,node_count_needed,
        req_deg_seq,unique_timestamps,undirected=True):
    
    unique_timestamps = np.array(list(unique_timestamps))
    ...
    #print(req_deg_seq)
    tedges = {}
    examined_timestamps = unique_timestamps[(unique_timestamps >= start) & (unique_timestamps < end)]
    for time in examined_timestamps:
        if time in graph:
            for start_node, adj_list in graph[time].items():
                for end_node, count in adj_list.items():
                    if start_node != end_node: 
...
def sample_adj_graph_multinomial_k_inductive_continuous(graph,start,end,edge_count_needed,node_count_needed,req_deg_seq,unique_timestamps,undirected=True,debug=False):
...
def get_adj_origina_graph_from_original_temporal_graph_continuous(temporal_original_graph,start_time,end_time, unique_timestamps):
    graph = defaultdict(lambda : defaultdict(lambda: 0))
    unique_timestamps = np.array(list(unique_timestamps))
    examined_timestamps = unique_timestamps[(unique_timestamps >= start_time) & (unique_timestamps < end_time)]
    for time in examined_timestamps:
        #print(time)
        for start_node,adj_list in temporal_original_graph[time].items():
            for end_node, value in adj_list.items():
...
def get_total_nodes_and_edges_from_temporal_adj_list_in_time_range_continuous(temporal_adj_matrix,start_time,end_time, unique_timestamps):
    ct = 0
    graph = defaultdict(lambda:defaultdict(lambda: 0))
    unique_nodes = set()
    unique_timestamps = np.array(list(unique_timestamps))
