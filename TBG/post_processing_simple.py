...
def post_process_label(data, label, generated_events, generated_times,
    ...
    daily_hcw_room_pairs = sample_snapshot_src_dst_pairs(
        adj_list_temporal_sampled, min_t, max_t, one_day_window,
        target_daily_edge_counts, topk_edge_sampling=True, 
        undirected=undirected
    )
    # evaluate the overlapping rate
    commons = defaultdict(dict)
    for day_idx in orig_daily_hcw_room_pairs.keys():
        common = orig_daily_hcw_room_pairs[day_idx].intersection(daily_hcw_room_pairs[day_idx])
        if len(common) > 0:
            commons[day_idx] = len(common) / len(orig_daily_hcw_room_pairs[day_idx])
    # percentage of common hcw-room pairs
    logging.info(f"Percentage of common hcw-room pairs: {np.median(list(commons.values()))}")
    
    # Step 2: fuse edges that are close in time
    if fuse_type == 'adaptive':
        hcw_room_pairs_events = adaptive_fuse_samepair_edges(
            adj_list_temporal_sampled, fuse_gap, is_hcw, vocab, undirected=undirected
        )
    elif fuse_type == 'snapshot':
        hcw_room_pairs_events = snapshot_fuse_samepair_edges(
            adj_list_temporal_sampled, fuse_gap, min_t, max_t, is_hcw, vocab, undirected=undirected
        )
    else:
        raise ValueError(f"Invalid fuse type: {fuse_type}")
    # filter out hcw-room pairs that are not in the daily graph
    # hcw_room_pairs_events = {pair: [e for e in events if pair in daily_hcw_room_pairs[e[0] // one_day_window]] for pair, events in hcw_room_pairs_events.items()}
    daily_fused_edges_graph = defaultdict(lambda: defaultdict(list))
    for (hcw, room), events in hcw_room_pairs_events.items():
        for count, time in events:
            day_idx = time // one_day_window
            if (hcw, room) in daily_hcw_room_pairs[day_idx]:
                daily_fused_edges_graph[day_idx][(hcw, room)].append((count, time))
    
    # Step 3: maintain the sparse graph structure
    # remove redundant visits (keep only visits that appear frequently - 
    # due to the nature of modeling random walks instead of modeling the exact graph)
    remain_graph = {}
    missing_day = []
    for start_time in range(min_t, max_t, one_day_window):
        day_idx = start_time // one_day_window
        daily_edges = daily_fused_edges_graph[day_idx]

        if len(daily_edges) == 0 and target_daily_edge_counts[day_idx] > 0:
            missing_day.append(day_idx)
            continue
        
        req_num_visits = target_daily_nvisits[day_idx]
        daily_hr_pairs_counts = {(hcw, room): len(visits)
                                for (hcw, room), visits in daily_edges.items()}
        
        # distribute the required number of visits to each hcw-room pair, 
        # ensuring each pair has at least one visit
        daily_hr_pairs_nvisits = {}
        less_than_one_visits_pairs = []
        # ww = [c / sum(daily_hr_pairs_counts.values()) for c in daily_hr_pairs_counts.values()]
        total_counts = sum(daily_hr_pairs_counts.values())
        for (hcw, room), pcount in daily_hr_pairs_counts.items():
            # nvi_pair = int(round_half_up(count / sum(daily_hr_pairs_counts.values()) * req_num_visits))
            nvi_pair =  int(np.round(pcount / total_counts * req_num_visits))
            daily_hr_pairs_nvisits[(hcw, room)] = max(1, nvi_pair)
            if nvi_pair < 1:
                less_than_one_visits_pairs.append((hcw, room))
        
        if less_than_one_visits_pairs:
            logging.info(f"HCW-room pairs with less than one visit: {less_than_one_visits_pairs}")
        # sample the required number of visits for each hcw-room pair
        if len(missing_day) > 0:
            logging.info(f"Missing visits day: {missing_day}")

        # subsample the visits for each hcw-room pair
        remain_graph[day_idx] = {}
        for (hcw, room), visits in daily_edges.items():
            pair_nvisits = daily_hr_pairs_nvisits[(hcw, room)]
            if pair_nvisits > 0:
                weights = [v[0] for v in visits]
                weights = [w / sum(weights) for w in weights]
                # sample visits without replacement
                if len(visits) < pair_nvisits:
                    sampled_visits = range(len(visits))
                else:
                    sampled_visits = np.random.choice(len(visits), pair_nvisits, 
                                                      replace=False, p=weights)
                
                remain_graph[day_idx][(hcw, room)] = [visits[i] for i in sampled_visits]

        # assertion
        n_remain_visits = sum([len(visits) for visits in remain_graph[day_idx].values()])
        if req_num_visits - n_remain_visits > len(daily_hr_pairs_counts):
            logging.info(f"Number of visits mismatch: {n_remain_visits} != {req_num_visits}")
            import pdb; pdb.set_trace()
            
    # Step 4: resovle conflicts
    hcw_std_graph = defaultdict(list)
    for day_idx in remain_graph:
        for (hcw, room), visits in remain_graph[day_idx].items():
            for count, start_time in visits:
                hcw_std_graph[hcw].append((room, count, start_time))

    # sort visits based on start time
    for hcw in hcw_std_graph:
        hcw_std_graph[hcw] = sorted(hcw_std_graph[hcw], key=lambda x: x[2])
    
    # resolve conflicts: when two visits start time is within min_gap, keep the one with higher count
    # e.g. e.g. (H1, R1, t1) and (H1, R2, t2) with t2 - t1 < min_gap, keep the one with higher count
    ...
    logging.info(f"Min, max, average number of visits per HCW in original data: "
                 f"{min_visits_per_hcw_orig}, {max_visits_per_hcw_orig}, {avg_visits_per_hcw_orig}")
    logging.info(f"HCWs with only one visit in original data: "
                 f"{[hcw for hcw in orig_hcw_visits if orig_hcw_visits[hcw] == 1]}")
...
if __name__ == '__main__': 
    ...
    target_daily_edge_counts, target_daily_node_counts, orig_daily_hcw_room_pairs = \
        extract_original_snapshot_stats(data, one_day_window, min_t, max_t, undirected)
    
    target_daily_nvisits = defaultdict(dict)
    # Precompute day indices for all rows in the data
    data['day_idx'] = data['days'] // one_day_window

    # Group data by label and day_idx for faster access
    grouped_data = data.groupby(['label', 'day_idx']).size()

    for label in target_daily_edge_counts.keys():
        for snapshot_idx in target_daily_edge_counts[label].keys():
            target_daily_nvisits[label][snapshot_idx] = \
    ...
    for i in range(0, num_of_sampled_graphs):
        ...
        def process_label(label):
            return post_process_label(data, label, generated_events[label], generated_times[label],
                          target_daily_edge_counts[label], target_daily_nvisits[label],
                          orig_daily_hcw_room_pairs[label], vocab, inv_vocab, l_w,
                          min_t, max_t, one_day_window, fuse_gap, fuse_type,
                          end_node_id, is_hcw, pp_save_dir, id_to_graph_label,
                          min_duration, max_duration, hcw_to_cluster,
                          undirected=True)
